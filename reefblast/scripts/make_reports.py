...
write_report("closed_forms", "Closed forms", [
    ("bubbly Hugoniot", wrap(
        ...
        " at p* = alpha K_l/(1 - alpha).  Exactly, U(p*) = c_l/((1 - alpha)"
        " sqrt(2 - eps)) with eps = (1 + p*/p0)^(-1/kappa); as p*/p0 grows"
        " this tends to c_l/(sqrt(2)(1 - alpha)), so the curves for"
    ...
    ("spall onset", wrap(
        ...
        " d_c = c_s theta artanh(Z_c/Z_s).  Failure first occurs at depth"
        " x* = (c_s theta/2) ln(1/(|R_b| - sigma_t/P_s)) behind the back"
        " face, the thickness of the spall scab; spall requires x* < d.")
     + [""] + dc_rows),
    ("bubble", wrap(
...
write_report("parameters", "Symbols, units, and reference values", [
    ...
    ("reference values (illustrative, not calibrated)", [
        ...
        f"  plates at {Z_REEF} m, charge directly above (normal"
        f" incidence); diel alpha_max = {ALPHA_MAX}",
...
if f5 is not None:
    ...
    notes += [("figure 5, damage regimes", wrap(
        f"Spall standoff of the 12 cm plate rises from {r12[0]:.2f} m at"
        f" alpha = {f5['al_b'][0]:.0e} to {r12[-1]:.2f} m at alpha ="
        f" {f5['al_b'][-1]:.2e}.  Crush standoff of the same plate falls from"
        ...
        "  d (m) | spall standoff at alpha min | at alpha max"] + [
        f"  {d:5.2f} | {f5[f'rs_{d:g}'][0]:20.3f} |"
        f" {f5[f'rs_{d:g}'][-1]:12.3f}" for d in D_FAMILY])]
d6 = csv("fig06_diel")
lines6 = []
spall_cols = [n for n in d6.dtype.names if n.startswith("R_spall")]
crush_cols = [n for n in d6.dtype.names if n.startswith("R_crush")]
for am, cs, cc in zip(ALPHA_MAX, spall_cols, crush_cols):
    lines6.append(f"  alpha_max = {am:g}: spall standoff"
...
write_report("open_items", "Open items and negative results", [
    ...
    ("the canopy is treated in its relaxed limit", wrap(
        "The secant impedance assumes canopy gas reaches pressure"
        " equilibrium inside the shock.  Figure 4 shows that it does not:"
        " with theta f_M of order one, a step front drives bubbles past"
        " equilibrium.  In a relaxing medium the leading edge travels near"
        " the frozen speed and sees almost no impedance contrast, over a"
        " relaxation length comparable to the canopy thickness.  The"
        " reported canopy effects are therefore the relaxed limit and are"
        " likely upper bounds on both front-face shielding and back-face"
        " tension early in the pulse.")),
    ("geometry", wrap(
        "Every range is a vertical standoff with the charge directly"
        " overhead.  Horizontal radii are not computed: beyond the critical"
        " angle arcsin(c_l/c_s) = 30 degrees no propagating compressional"
        " wave enters an acoustic skeleton, and loading there runs through"
        " shear and flexural waves that the model does not represent.")),
    ("cavitation above the canopy", wrap(
        "The wave reflected from the canopy top is negative.  Above the"
        " interface, incident tail plus reflection turns tensile at a"
        " height of order (c_l theta/2) ln(1/|R_wc|), a few centimetres,"
        " with MPa amplitude, so the water there cavitates in practice."
        "  That makes later reverberations lossy and nonlinear; the"
        " impulse invariant holds for the linear model only.")),
    ("strength and source constants", wrap(
        "Static tensile strength understates the dynamic spall strength of"
        " brittle porous solids at strain rates of order 1e3 per second, so"
        " spall standoffs are likely upper bounds.  The Cole similitude"
        " constants were fitted to charges far larger than bottle charges;"
        " constants fitted to charges of order one kilogram would be more"
        " appropriate and should bracket the results.")),
    ("assumptions carried by the whole construction", wrap(
        "Canopy impedance frozen at the"
        " secant value for the incident peak, for both loading and"
        " unloading, and for the canopy above and below the plate; the"
        " canopy under the plate is in fact loaded less, so this choice"
        " understates the back-face effect.  No"
